...
class UserAdmin(BaseUserAdmin):
    ...
    list_display = ('username', 'email', 'first_name', 'last_name', 'is_staff', 'is_superuser', 'get_assigned_service')
    list_filter = ('is_staff', 'is_superuser', 'staff_profile__role', 'staff_profile__assigned_service')
...
class ServiceAdmin(admin.ModelAdmin):
    list_display = ('name', 'prefix', 'is_active', 'windows_count', 'waiting_count', 'currently_serving_display')
...
class ServiceWindowAdmin(admin.ModelAdmin):
    list_display = ('service', 'window_number', 'name', 'status', 'current_staff_display', 'is_available')
...
class TicketAdmin(admin.ModelAdmin):
    list_display = ('display_number', 'service', 'status', 'assigned_window_display', 'created_at_short', 'people_ahead', 'is_today')
    list_filter = ('status', 'service', 'ticket_date', 'created_at', 'assigned_window')
    search_fields = ('display_number', 'ticket_id', 'service__name')
    readonly_fields = ('ticket_id', 'queue_number', 'display_number', 'ticket_date', 'created_at', 'called_at', 'served_at', 'people_ahead_display', 'wait_time_display')
    ...
    display_number.short_description = 'Ticket No.'
    ...
    def get_queryset(self, request):
        # Optimize queries
        return super().get_queryset(request).select_related('service', 'assigned_window', 'called_by', 'served_by')